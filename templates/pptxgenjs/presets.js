...
const { loadPulsePreset } = require('./pulse_theme.js');

// The house style. Built from pulse/theme.json rather than written here, so
// designers change colours, fonts, sizes and spacing in that one file.
const PULSE_PRESET_NAME = 'pulse-library';

const PRESETS = {
  [PULSE_PRESET_NAME]: loadPulsePreset(),
  'executive-clinical': {
    bg: 'F4F8FB',
    bg_dark: '071E3A',
    surface: 'FFFFFF',
...
const DEFAULT_PRESET_NAME = PULSE_PRESET_NAME;
...
// can select one: a named preset is accepted, reported, then overridden.
// Set to null to hand the choice back.
const FORCED_PRESET_NAME = PULSE_PRESET_NAME;
const forcedWarnings = new Set();

function forcePresetName(requested) {
  if (!FORCED_PRESET_NAME) return String(requested || '').trim().toLowerCase();
  const key = String(requested || '').trim().toLowerCase();
  if (key && key !== FORCED_PRESET_NAME && !forcedWarnings.has(key)) {
    forcedWarnings.add(key);
    console.warn(
      `[pptxgenjs] запрошена тема '${key}'; этот форк собирает всё в ` +
      `'${FORCED_PRESET_NAME}'. См. FORCED_PRESET_NAME в ` +
      'templates/pptxgenjs/presets.js.',
    );
  }
  return FORCED_PRESET_NAME;
}
...
function getPreset(name) {
  const key = forcePresetName(name);
