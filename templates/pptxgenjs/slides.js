...
// ---------------------------------------------------------------------------
// Corporate template geometry ("Пульс").
//
// The renderer authors every deck in a 10 x 5.625in space; the corporate
// template is 13.333 x 7.5in. Both are 16:9, so the two spaces differ by a
// constant 4/3 and a deck built here is visually identical to one built at
// template size -- a 24pt line on a 13.333in slide and an 18pt line on a 10in
// slide occupy the same fraction of the page.
//
// Every constant below is the template value divided by CORP_SCALE, so the
// source of each number stays checkable against ppt/slideLayouts/*.xml in
// corporate/theme-source.md.
// ---------------------------------------------------------------------------
const CORP_SCALE = 4 / 3;
const corp = (templateInches) => templateInches / CORP_SCALE;

// Geometry and type come from pulse/theme.json, which is the file designers
// edit. Nothing here holds a value of its own; PULSE is the loaded theme
// expressed in the renderer's 10 x 5.625in space.
const pulseTheme = require('./pulse_theme.js');
const PULSE = pulseTheme.loadPulsePreset();

const CORP = {
  margin: PULSE.pulse_grid.margin,
  titleY: PULSE.pulse_grid.titleY,
  titleH: PULSE.pulse_grid.titleH,
  eyebrowY: PULSE.pulse_grid.subtitleY,
  eyebrowH: PULSE.pulse_grid.subtitleH,
  bodyY: PULSE.pulse_grid.contentY,
  bodyH: PULSE.pulse_grid.contentBottom - PULSE.pulse_grid.contentY,
  contentW: PULSE.pulse_grid.contentW,
  colW: (PULSE.pulse_grid.contentW - PULSE.pulse_grid.gutter) / 2,
  col2X: PULSE.pulse_grid.margin + (PULSE.pulse_grid.contentW + PULSE.pulse_grid.gutter) / 2,
  gutter: PULSE.pulse_grid.gutter,
  logoX: PULSE.pulse_grid.margin,
  logoY: PULSE.pulse_grid.footerY,
  logoW: corp(1.34),
  logoH: corp(0.25),
  pageNumX: PULSE.pulse_grid.pageNumX,
  pageNumY: PULSE.pulse_grid.footerY,
  pageNumW: PULSE.pulse_grid.pageNumW,
  pageNumH: PULSE.pulse_grid.footerH,
  titlePt: PULSE.pulse_type.title,
  eyebrowPt: PULSE.pulse_type.subtitle,
  bodyPt: PULSE.pulse_type.body,
  captionPt: PULSE.pulse_type.caption,
};

// ---------------------------------------------------------------------------
// Optional per-deck type ramp.
//
// Font sizes are set as bare literals in ~170 places across this file, so
// there is no single constant to retune for a preset whose type scale differs
// from the house scale. Instead every size passes through textOpts(), which
// applies the active ramp. A ramp is a monotone piecewise-linear map from the
// house size to the preset's, so relative hierarchy inside a slide survives
// while the absolute scale moves onto the preset's own steps.
//
// null (the default) means "house scale", and no existing preset changes.
// ---------------------------------------------------------------------------
let TYPE_RAMP = null;

function setTypeRamp(anchors) {
  ...
  TYPE_RAMP = anchors
    ...
    .sort((a, b) => a[0] - b[0]);
  if (TYPE_RAMP.length < 2) TYPE_RAMP = null;
}

function rampFontSize(pt) {
  const size = Number(pt);
  if (!TYPE_RAMP || !Number.isFinite(size)) return pt;
  const first = TYPE_RAMP[0];
  const last = TYPE_RAMP[TYPE_RAMP.length - 1];
  // Outside the anchor range, hold the end ratio rather than the end value, so
  // an unusually large or small literal still moves in the right direction.
  if (size <= first[0]) return round1(size * (first[1] / first[0]));
  if (size >= last[0]) return round1(size * (last[1] / last[0]));
  for (let i = 1; i < TYPE_RAMP.length; i += 1) {
    const [x0, y0] = TYPE_RAMP[i - 1];
    const [x1, y1] = TYPE_RAMP[i];
    if (size <= x1) {
      const t = x1 === x0 ? 0 : (size - x0) / (x1 - x0);
...
// Text that already carries a preset-specific size (a corporate header, say)
// must not be remapped a second time. Callers mark those with rampExempt.
function exactFontSize(pt) {
  return { fontSize: pt, __rampExempt: true };
}
...
// ---------------------------------------------------------------------------
// User-visible label defaults.
//
// The renderer prints a handful of words of its own -- footer labels, the
// "Focus" rail heading, a caption prefix, placeholder titles. They were
// English literals at the call sites, which is fine for a deck written in
// English and wrong for one that is not: a Russian deck came out with
// "Sources:", "Refs:", "FOCUS" and "Source:" in it.
//
// A preset supplies overrides via preset.labels; anything it omits falls back
// to the English default, so no existing preset changes.
// ---------------------------------------------------------------------------
const DEFAULT_LABELS = {
  ...
  section: 'Section',
  noFacts: 'No facts provided.',
  evidencePanel: 'Evidence panel',
  deckAuthor: 'Presentation Team',
  decision: 'Decision',
  evidenceTrigger: 'Evidence trigger',
  owner: 'Owner',
  timing: 'Timing / caveat',
};

function label(preset, key) {
  const overrides = preset && preset.labels && typeof preset.labels === 'object'
    ? preset.labels
    : {};
  const value = overrides[key];
  return typeof value === 'string' && value.trim() ? value : DEFAULT_LABELS[key];
}
...
function textOpts(extra) {
  ...
  // Single choke point for the per-deck type ramp -- see setTypeRamp above.
  if (opts.__rampExempt) {
    delete opts.__rampExempt;
  } else {
    opts.fontSize = rampFontSize(opts.fontSize);
  }
  return opts;
}
...
// Some presets are light-only: the corporate template has no dark slide among
// its 71, so a panel that the house style paints in bg_dark has to become a
// tinted plate instead. Surface and ink are decided together here, because
// changing one without the other is how a deck ends up with white text on a
// white plate.
// Bottom of the usable content band. House presets take the full canvas and
// subtract a per-variant slice for footer chrome. A preset that pins its own
// content floor (the corporate template ends its body band well above the
// wordmark) uses that instead, so nothing runs into the master furniture.
// The corporate template draws square-cornered plates. A preset can say so
// once here rather than at every call site that reaches for a rounded card.
function plateShape(preset) {
  return preset && preset.square_plates ? 'rect' : 'roundRect';
}

function contentBottom(preset, houseReserve) {
  const floor = Number(preset && preset.content_floor);
  if (Number.isFinite(floor) && floor > 0) return floor;
  return SLIDE_H - (Number(houseReserve) || 0);
}

function isLightOnly(preset) {
  return Boolean(preset && preset.light_only);
}

function darkSurface(preset, fallback) {
  return isLightOnly(preset)
    ? cleanHex(preset.surface, 'EAF2FF')
    : cleanHex(preset.bg_dark, fallback || '0F172A');
}

function onDarkInk(preset, fallback) {
  return isLightOnly(preset)
    ? cleanHex(preset.text, '000000')
    : (fallback || 'FFFFFF');
}
...
function roleBodyBox(header, slideData, preset, opts = {}) {
  ...
  return {
    ...
    h: Math.max(1.0, contentBottom(preset, footerReserve) - y - summaryReserve),
  };
}
...
function addPageSystemChrome(slide, preset, slideData) {
  ...
  if (system === 'pulse-plate') {
    // The corporate template carries no rails, brackets or corner furniture:
    // its slide master holds a wordmark bottom-left and a slide number
    // bottom-right, and nothing else. Reproducing that means drawing less,
    // not more -- the only page-level chrome here is the wordmark, placed at
    // the master's own coordinates. The slide number stays in addFooter with
    // the rest of the footer text.
    addCorporateWordmark(slide);
  }
}

// Resolved once: the wordmark extracted from the corporate template, kept in
// the repository as a theme asset (see corporate/assets/).
const CORPORATE_WORDMARK = path.join(__dirname, '..', '..', 'pulse', 'assets', 'logo.png');

function addCorporateWordmark(slide) {
  // The template carries the wordmark on its slide master, so it is on every
  // slide without exception. Renderers that skip addPageSystemChrome (kpi-hero
  // paints its own full-slide background, for one) would otherwise drop it, so
  // this is idempotent and the dispatcher calls it for every slide.
  if (slide.__corporateWordmark) return;
  if (!fs.existsSync(CORPORATE_WORDMARK)) return;
  slide.__corporateWordmark = true;
  slide.addImage({
    path: CORPORATE_WORDMARK,
    x: CORP.logoX,
    y: CORP.logoY,
    w: CORP.logoW,
    h: CORP.logoH,
    altText: 'Пульс',
    objectName: 'metadata:corporate-wordmark',
  });
}
...
function addDarkTitleBar(slide, preset, title, subtitle, slideData = {}) {
  ...
  if (headerMode === 'pulse-plate') {
    // Fixed bands, not measured ones. The corporate template pins its title,
    // subheading and body to the same three y-offsets on every layout, and
    // that repetition is most of what makes a deck read as "ours" -- so the
    // header does not reflow around a long title the way the house header
    // does. Long titles shrink to fit their band instead.
    const titleText = safeText(title, 'Untitled');
    const eyebrowText = safeText(subtitle);
    const ink = cleanHex(preset.text, '000000');

    slide.addText(titleText, textOpts(Object.assign({
      x: CORP.margin,
      y: CORP.titleY,
      w: CORP.contentW,
      h: CORP.titleH,
      ...
      valign: 'top',
      fit: 'shrink',
      objectName: 'title:slide-title',
    }, exactFontSize(CORP.titlePt))));

    if (eyebrowText) {
      slide.addText(eyebrowText, textOpts(Object.assign({
        x: CORP.margin,
        y: CORP.eyebrowY,
        w: CORP.contentW,
        h: CORP.eyebrowH,
      ...
      }, exactFontSize(CORP.eyebrowPt))));
    }

    return Object.assign({}, metrics, {
      barH: CORP.titleY + CORP.titleH,
      stripeY: CORP.titleY + CORP.titleH,
      // Body starts at the template's own body offset when a subheading is
      // present, and takes the subheading band back when it is not.
      contentTop: eyebrowText ? CORP.bodyY : CORP.eyebrowY,
      textW: CORP.contentW,
      titleText,
      subtitleText: eyebrowText,
      titleFont: CORP.titlePt,
      subtitleFont: CORP.eyebrowPt,
      titleY: CORP.titleY,
      titleH: CORP.titleH,
      subtitleY: CORP.eyebrowY,
      subtitleH: eyebrowText ? CORP.eyebrowH : 0,
    });
  }
...
function footerChromeModel(slideData, preset) {
  ...
  const sourceLabel = safeText(slideData.source_label || preset.footer_source_label, label(preset, 'sources'));
  const refsLabel = safeText(slideData.refs_label || preset.footer_refs_label, label(preset, 'refs'));
...
// Called once per slide by the dispatcher, after the variant renderer has
// painted whatever background it wanted.
function ensurePageChrome(slide, preset) {
  const system = String(preset && preset.page_system || '').trim().toLowerCase();
  if (system === 'pulse-plate') addCorporateWordmark(slide);
}
...
function addFooter(slide, preset, slideData) {
  ...
  if (footerMode === 'pulse-plate') {
    // No separator rule: the corporate master has none. Footer text starts
    // clear of the wordmark drawn by the page system, and the slide number
    // sits in the master's own slide-number box.
    const textX = CORP.logoX + CORP.logoW + 0.14;
    const textRight = showPageNumber && pageNumber ? CORP.pageNumX - 0.14 : SLIDE_W - CORP.margin;
    const parts = [];
    if (footer) parts.push(footer);
    parts.push(...provenanceParts);
    const text = parts.join(' · ');
    if (text && textRight > textX) {
      slide.addText(text, textOpts(Object.assign({
        x: textX,
        y: CORP.logoY - 0.06,
        w: textRight - textX,
        h: CORP.logoH + 0.12,
      ...
      }, exactFontSize(CORP.captionPt))));
    }
    if (showPageNumber && pageNumber) {
      // The corporate master shows a bare slide number, not "n/N".
      const bare = String(pageNumber).split('/')[0];
      slide.addText(bare, textOpts(Object.assign({
        x: CORP.pageNumX,
        y: CORP.pageNumY,
        w: CORP.pageNumW,
        h: CORP.pageNumH,
        fontFace: preset.font_body,
        color: cleanHex(preset.text, '000000'),
        align: 'right',
        valign: 'middle',
...
function renderTitleSplit(pptx, slide, slideData, preset) {
  ...
  const titleText = safeText(slideData.title, label(preset, 'untitledDeck'));
...
function renderTitleCorporatePlate(pptx, slide, slideData, preset) {
  // Cover slide in the corporate idiom: white field, left-aligned black
  // wordmark-scale title, one accent rule, subtitle in the brand blue. The
  // template spends its cover on space, so this layout does too -- no plate,
  // no gradient, no motif.
  const ink = cleanHex(preset.text, '000000');
  const accent = cleanHex(preset.accent_primary, '005AFE');
  paintBackground(slide, cleanHex(preset.bg, 'FFFFFF'));

  const titleText = safeText(slideData.title, label(preset, 'untitled'));
  const subtitleText = safeText(slideData.subtitle);
  const kicker = safeText(slideData.kicker || slideData.eyebrow);

  let y = corp(2.10);
  if (kicker) {
    slide.addText(kicker.toUpperCase(), textOpts(Object.assign({
      x: CORP.margin,
      y,
      w: CORP.contentW,
      h: corp(0.40),
      ...
      color: accent,
      charSpacing: 1.2,
      objectName: 'support:title-kicker',
    }, exactFontSize(CORP.captionPt * 1.4))));
    y += corp(0.52);
  }

  // 40pt on the template's 13.333in page, the step above a slide title.
  const titlePt = 40 / CORP_SCALE;
  slide.addText(titleText, textOpts(Object.assign({
    x: CORP.margin,
    y,
    w: corp(10.50),
    ...
    valign: 'top',
    fit: 'shrink',
    objectName: 'title:deck-title',
  }, exactFontSize(titlePt))));

  const ruleY = y + corp(2.05);
  slide.addShape('rect', shapeOpts({
    x: CORP.margin,
    y: ruleY,
    w: corp(1.60),
  ...
  if (subtitleText) {
    slide.addText(subtitleText, textOpts(Object.assign({
      x: CORP.margin,
      y: ruleY + corp(0.28),
      w: corp(9.20),
      h: corp(0.80),
      ...
      valign: 'top',
      fit: 'shrink',
      objectName: 'support:deck-subtitle',
    }, exactFontSize(CORP.eyebrowPt))));
  }
...
function renderContractStage(slide, slideData, preset, contract, kind) {
  ...
  const title = safeText(slideData.title, kind === 'title' ? label(preset, 'untitledDeck') : label(preset, 'section'));
  const subtitle = safeText(slideData.subtitle);
...
function renderTitle(pptx, slide, slideData, preset) {
  ...
  const layout = String(slideData.title_layout || roleLayout || preset.title_layout || 'split-hero')
    .trim()
    .toLowerCase();
  if (layout === 'pulse-plate') {
    renderTitleCorporatePlate(pptx, slide, slideData, preset);
  } else if (layout === 'lab-plate') {
...
function renderRoleSection(slide, slideData, preset) {
  ...
  const title = safeText(slideData.title, label(preset, 'section'));
  const subtitle = safeText(slideData.subtitle);
  const accent = cleanHex(preset.accent_primary, '1493A4');
  const secondary = cleanHex(preset.accent_secondary, accent);
...
function renderSection(pptx, slide, slideData, preset) {
  ...
  if (String(preset.page_system || '').trim().toLowerCase() === 'pulse-plate') {
    // Not one of the 71 slides in the corporate template uses a dark or
    // full-bleed background, so a section divider stays on the white field
    // and separates by scale and space instead of by inversion.
    paintBackground(slide, cleanHex(preset.bg, 'FFFFFF'));
    const ink = cleanHex(preset.text, '000000');
    const accent = cleanHex(preset.accent_primary, '005AFE');
    slide.addShape('rect', shapeOpts({
      x: CORP.margin, y: corp(2.30), w: corp(1.60), h: corp(0.06),
      fill: { color: accent }, line: { color: accent, width: 0 },
    }));
    slide.addText(title, textOpts(Object.assign({
      x: CORP.margin, y: corp(2.66), w: corp(10.50), h: corp(1.40),
      fontFace: preset.font_heading, bold: true, color: ink,
      valign: 'top', fit: 'shrink', objectName: 'title:section-title',
    }, exactFontSize(32 / CORP_SCALE))));
    if (subtitle) {
      slide.addText(subtitle, textOpts(Object.assign({
        x: CORP.margin, y: corp(4.20), w: corp(9.20), h: corp(0.72),
        fontFace: preset.font_body, color: cleanHex(preset.text_muted, '5D6578'),
        valign: 'top', fit: 'shrink', objectName: 'support:section-subtitle',
      }, exactFontSize(CORP.eyebrowPt))));
    }
    addCorporateWordmark(slide);
    addFooter(slide, preset, slideData);
...
function renderStandard(pptx, slide, slideData, preset) {
  ...
  if (decisionContract) {
    ...
    const fallbackTitles = ['decision', 'evidenceTrigger', 'owner', 'timing']
      .map((key) => label(preset, key));
  ...
  if (referencesContract) {
    ...
    if (rows.length) {
      renderTableContract(
        ...
        header,
        {
          title: safeText(slideData.title, label(preset, 'sources')),
  ...
  const contentH = contentBottom(preset, 0.55) - contentY - summaryReserve;
  ...
  if (hasHighlights) {
    ...
    slide.addShape(plateShape(preset), shapeOpts({
...
function renderCards(pptx, slide, slideData, preset, columns) {
  ...
  const maxCardH = contentBottom(preset, 0.65) - cardY;
...
function renderSplit(pptx, slide, slideData, preset) {
  ...
  const contentH = contentBottom(preset, 0.60) - contentY;
  const usableW = SLIDE_W - MARGIN_X * 2;
  ...
  const highlights = Array.isArray(slideData.highlights)
    ? slideData.highlights.map((h) => safeText(h)).filter(Boolean)
    : [];
  const highlightsLabel = safeText(slideData.highlights_label, label(preset, 'focus'));
  slide.addText(highlightsLabel.toUpperCase(), textOpts({
    x: rightX + 0.3,
    y: contentY + 0.25,
    w: rightW - 0.5,
    h: 0.30,
...
function renderTimelineChapterSpread(pptx, slide, slideData, preset) {
  ...
  const contentY = header.contentTop + 0.34;
  const contentH = contentBottom(preset, 0.70) - contentY;
  const usableW = SLIDE_W - MARGIN_X * 2;
...
function renderTimeline(pptx, slide, slideData, preset) {
  ...
  rawItems.forEach((item, idx) => {
    ...
    const cardH = contentBottom(preset, 0.65) - cardY;
    slide.addShape('rect', shapeOpts({
...
function renderRoleFactCard(slide, preset, fact, box, index, isAnchor, dense) {
  ...
  const fill = darkAnchor ? darkSurface(preset) : (preset.surface || 'FFFFFF');
  const titleColor = darkAnchor
    ? onDarkInk(preset)
...
function renderStatsFeatureLeft(slide, slideData, preset, header, facts, iconPaths) {
  const contentY = header.contentTop + 0.30;
  const contentH = contentBottom(preset, 0.72) - contentY;
  const usableW = SLIDE_W - MARGIN_X * 2;
  const gutter = 0.32;
...
function renderStats(pptx, slide, slideData, preset) {
  ...
  if (facts.length === 0) {
    slide.addText(label(preset, 'noFacts'), textOpts({
      x: MARGIN_X,
  ...
  const tileH = contentBottom(preset, 0.75) - tileY;
  ...
  facts.forEach((fact, idx) => {
    ...
    if (fact.source) {
      slide.addText(label(preset, 'sourcePrefix') + fact.source, textOpts({
        x: tx + 0.25,
        y: tileY + tileH - 0.30,
        w: tileW - 0.4,
        h: 0.22,
...
function renderKpiHero(pptx, slide, slideData, preset) {
  // Inverted by default, but a light-only preset has no dark slide to invert
  // to -- the corporate template never fills a slide edge to edge.
  const dark = isLightOnly(preset) ? false : slideData.theme !== 'light';
  ...
  const contentH = contentBottom(preset, 0.85) - effectiveContentTop;
...
function renderComparisonScorecard(pptx, slide, slideData, preset) {
  ...
  const bottom = contentBottom(preset, footerReserve) - verdictH - verdictGap;
  const scoreH = 0.78;
...
function renderComparison2col(pptx, slide, slideData, preset) {
  ...
  const colBottom = contentBottom(preset, 0.65) - verdictH - verdictGap;
...
function renderScientificLedgerRail(slide, slideData, preset, figures, metrics) {
  ...
  rows.forEach((figure, idx) => {
    ...
    slide.addText(`${figure.label}. ${truncate(figure.title || label(preset, 'evidencePanel'), 28)}`, textOpts({
      x: MARGIN_X + 0.16,
      y: y + 0.12,
      w: ledgerW - 0.28,
      h: 0.28,
...
// Legend and value labels are a house convention, not a per-slide decision, so
// a preset can set them once. The outline still wins: options.showLegend /
// options.showValue are honoured whenever the slide states them.
// The panel a chart sits on. preset.surface is right for cards and callouts
// but not here: all 28 charts in the corporate template sit on a transparent
// background with no frame, so a preset can name a different chart panel
// without giving up its tinted plates elsewhere.
function chartPanelFill(preset) {
  return cleanHex(preset.chart_surface || preset.surface, 'FFFFFF');
}

function chartPanelHasFrame(preset) {
  return !(preset && preset.chart_frameless);
}
...
function chartShowValue(options, preset) {
  if (options.showValue !== undefined) return Boolean(options.showValue);
  if (preset && preset.chart_show_value !== undefined) return Boolean(preset.chart_show_value);
  return false;
}
...
function chartTypeForPayload(pptx, payload) {
  ...
  // pptxgenjs supports all of these natively. Mapping doughnut onto pie and
  // area onto bar silently produced a different chart than the outline asked
  // for -- a filled circle where a ring was meant, and columns where an area
  // was meant -- with nothing in the output to say so.
  const supported = {
    line: 'line',
    pie: 'pie',
    doughnut: 'doughnut',
  ...
  const key = supported[raw];
  if (key) return types[key] || key;
  console.warn(`[pptxgenjs] unknown chart type '${raw}'; rendering as 'bar'.`);
  return types.bar || 'bar';
}

// cleanHex substitutes its own dark-slate default when handed a falsy
// fallback, so cleanHex(undefined, '') returns '0F172A' rather than nothing.
// chartColors used that idiom to mean "skip this one", which instead pushed
// 0F172A onto the front of every palette: series 1 of every chart, in every
// preset, was rendered dark slate instead of the preset's own accent.
...
function chartColors(payload, preset) {
  ...
  ['color1', 'color2', 'color3', 'color4'].forEach((key) => {
    const color = optionalHex(payload && payload[key]);
    if (color && !colors.includes(color)) colors.push(color);
  });
  // A preset may carry its own series ramp. The generic fallback below ends on
  // bg_dark, which is wrong for a light-only preset -- it puts a near-black
  // wedge in the middle of a white deck -- and it only yields four colours,
  // so a fifth series repeats the first.
  if (Array.isArray(preset.chart_series)) {
    preset.chart_series.forEach((value) => {
      const color = optionalHex(value);
      if (color && !colors.includes(color)) colors.push(color);
    });
  }
  const fallback = isLightOnly(preset)
    ? [preset.accent_primary, preset.accent_secondary, preset.text_muted]
...
function renderChartContract(pptx, slide, slideData, preset, payload, facts, note, contract) {
  ...
  slide.addShape('rect', shapeOpts({
    ...
    fill: { color: chartPanelFill(preset) },
    line: {
      color: preset.line || 'CBD5E1',
      width: (policyLayout || !chartPanelHasFrame(preset)) ? 0 : 0.6,
      transparency: chartPanelHasFrame(preset) ? 0 : 100,
    },
  ...
  if (payload.__error__ || !Array.isArray(payload.series) || !payload.series.length) {
    ...
    const chartOptions = {
      ...
      showLegend: chartShowLegend(options, preset, series, payload),
      ...
      showValue: chartShowValue(options, preset),
      showTitle: false,
      showCatName: false,
      showSerName: false,
...
function renderChart(pptx, slide, slideData, preset) {
  ...
  if (sparseWide) {
    ...
    slide.addShape('rect', shapeOpts({
      ...
      line: {
        color: preset.line || 'CBD5E1',
        width: chartPanelHasFrame(preset) ? (chartTreatment === 'minimal' ? 0.35 : 0.65) : 0,
        transparency: chartPanelHasFrame(preset)
          ? (chartTreatment === 'minimal' ? 45 : 0)
          : 100,
      },
    }));
  }
...
function renderGeneratedImage(pptx, slide, slideData, preset) {
  ...
  const contentH = contentBottom(preset, 0.56) - contentY;
  const panelW = 3.05;
  const gutter = 0.28;
...
function addSummaryCallout(pptx, slide, slideData, preset) {
  ...
  if (mode === 'pulse-plate') {
    // Sits at the foot of the template's body band, not the foot of the
    // slide: below that line the corporate master belongs to the wordmark
    // and the slide number, and a callout there collides with both.
    const h = corp(0.62);
    const y = CORP.bodyY + CORP.bodyH - h;
    slide.addShape('rect', shapeOpts({
      x: CORP.margin,
      y,
      w: CORP.contentW,
      h,
      fill: { color: cleanHex(preset.surface, 'EAF2FF') },
      line: { color: cleanHex(preset.surface, 'EAF2FF'), width: 0 },
    }));
    slide.addShape('rect', shapeOpts({
      x: CORP.margin,
      y,
    ...
    slide.addText(text, textOpts(Object.assign({
      x: CORP.margin + corp(0.30),
      y: y + corp(0.08),
      w: CORP.contentW - corp(0.60),
      h: h - corp(0.16),
      fontFace: preset.font_body,
      bold: true,
      color: cleanHex(preset.text, '000000'),
      valign: 'middle',
      fit: 'shrink',
      objectName: 'support:summary-callout',
    }, exactFontSize(CORP.bodyPt))));
    return;
  }
