...
from design_tokens import PRESETS, FORCED_STYLE_PRESET, force_style_preset
    ...
    *,
    ...
    return {
        ...
        "readability_contract": {
            # From pulse/theme.json, in the renderer's own point space -- QA
            # measures the built package, so the floors follow the theme scale.
            **_pulse_readability(),
            "max_title_lines": 2,
...
def _pulse_readability() -> dict[str, float]:
    """Readability floors derived from the theme's own type scale."""
    scale = _pulse.load_theme()["type_scale_pt"]
    return {
        "min_title_pt": _pulse.points(scale["title"]),
        "min_body_pt": _pulse.points(scale["secondary"]),
        "min_caption_pt": _pulse.points(scale["micro"]),
    }
...
def main() -> int:
    ...
    if FORCED_STYLE_PRESET and requested_style_preset != "auto":
        requested_style_preset = force_style_preset(requested_style_preset)
        args.style_preset = requested_style_preset
    style_was_auto = requested_style_preset == "auto"
    if style_was_auto:
        # Upstream routed by topic here. This fork has one house style, and the
        # router matched English keywords only, so "auto" means the house style.
        ...
        style_selection = {
            ...
            "selection_basis": "forced_house_style",
            "topic_terms": [],
        }
    else:
        style_selection = {
