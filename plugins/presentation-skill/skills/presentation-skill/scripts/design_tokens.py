...
import pulse_theme as _pulse

#: Name of the one style this fork renders.
PULSE_STYLE_PRESET = "pulse-library"
...
def _pulse_style_preset() -> StylePreset:
    """Build the house preset from pulse/theme.json."""
    theme = _pulse.load_theme()
    c, s, g = theme["colors"], theme["type_scale_pt"], theme["grid_in"]
    return StylePreset(
        name=PULSE_STYLE_PRESET,
        palette={
            "bg_primary": c["bg"],
            "bg_dark": c["ink"],
            "surface": c["surface"],
            ...
            "accent_secondary": c.get("accent_bright", c["accent"]),
            "line": c["line"],
        },
        typography=TypographyTokens(
            title_max=float(s["hero"]),
            title_min=float(s["title"]),
            section_max=float(s["title"]),
            section_min=float(s["subtitle"]),
            body_max=float(s["subtitle"]),
            body_min=float(s["secondary"]),
            caption_max=float(s["caption"]),
            caption_min=float(s["micro"]),
        ),
        layout=LayoutTokens(
            margin_x=float(g["margin"]),
            top_safe=float(g["title_y"]),
            bottom_safe=round(
                float(theme.get("slide", {}).get("height_in", 7.5))
                - float(g["content_bottom"]), 3),
            gutter=float(g["gutter"]),
            rhythm=float(g.get("block_gap", 0.22)),
            rail_height=0.08,
            radius_style="square",
            edge_tolerance=0.06,
...
PRESETS: dict[str, StylePreset] = {
    # House style, built from pulse/theme.json rather than written here, so
    # designers change colours, fonts and spacing in that one file.
    "pulse-library": _pulse_style_preset(),
...
# This fork renders one house style and nothing else. The upstream presets stay
# in PRESETS so catalogs, treatment maps and tests still resolve, but no route
# can select one: a named preset is accepted, reported on stderr, and then
# overridden. Set to None to hand the choice back.
#
# The JS renderer has the matching switch in templates/pptxgenjs/presets.js
# (FORCED_PRESET_NAME); change both together or the two paths disagree.
FORCED_STYLE_PRESET = PULSE_STYLE_PRESET

_forced_warnings: set[str] = set()


def force_style_preset(name: str | None) -> str:
    """Return the preset that will actually be used, warning once per override."""
    key = str(name or "").strip().lower()
    if not FORCED_STYLE_PRESET:
        return key
    if key and key != FORCED_STYLE_PRESET and key not in _forced_warnings:
        _forced_warnings.add(key)
        print(
            f"[design_tokens] запрошена тема {key!r}; этот форк собирает всё в "
            f"{FORCED_STYLE_PRESET!r}. См. FORCED_STYLE_PRESET в "
            "scripts/design_tokens.py.",
            file=sys.stderr,
        )
    return FORCED_STYLE_PRESET
