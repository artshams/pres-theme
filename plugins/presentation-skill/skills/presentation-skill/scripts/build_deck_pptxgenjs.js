...
const { getPreset, DEFAULT_PRESET_NAME, listPresets, forcePresetName, FORCED_PRESET_NAME } = require(path.join(TEMPLATES_DIR, 'presets.js'));
const slides = require(path.join(TEMPLATES_DIR, 'slides.js'));
const { applySlotAliases, isPayloadEmpty } = require('../templates/pptxgenjs/slot_aliases.js');
const {
...
const FONT_PAIRS = {
  ...
  pulse_sb_v1: (() => {
    const p = require('../templates/pptxgenjs/pulse_theme.js').loadPulsePreset();
    return { font_heading: p.font_heading, font_body: p.font_body };
  })(),
};
...
const PALETTE_LIBRARY = {
  ...
  // Mirrors pulse/theme.json so the two never drift. Under the forced house
  // style deck_style.palette_key is ignored anyway; this entry exists so the
  // palette catalog stays complete.
  pulse_library_v1: (() => {
    const p = require('../templates/pptxgenjs/pulse_theme.js').loadPulsePreset();
    return {
      bg: p.bg,
      ...
      line: p.line,
      title_subtitle_color: p.surface_strong,
      section_subtitle_color: p.surface_strong,
    };
  })(),
...
const PRESET_TREATMENTS = {
  // Corporate template. The template has no rails, brackets, tinted plates or
  // corner furniture -- a slide is a title, a subheading, the content, and a
  // wordmark. The treatment values below therefore pick the quietest option
  // available at every slot; the visual identity comes from the fixed bands
  // and the type scale, not from applied chrome.
  'pulse-library': {
    page_system: 'pulse-plate',
    structural_motif: 'none',
    header_mode: 'pulse-plate',
    title_layout: 'pulse-plate',
    title_motif: 'none',
    section_motif: 'none',
    timeline_mode: 'open-events',
    matrix_mode: 'open-quadrants',
    stats_mode: 'tiles',
    // Equal, aligned columns. staggered-row varies the vertical silhouette to
    // fight sameness across a house deck; the corporate template has the
    // opposite instinct and repeats one grid on every slide.
    cards_mode: 'aligned-row',
    ...
    summary_callout_mode: 'pulse-plate',
    footer_mode: 'pulse-plate',
...
// corporate-pulse is deliberately absent: a grammar attaches role_systems,
// and those render through renderContractStage with their own geometry, which
// would override the template's fixed title/subheading/body bands. The preset
// carries its own layout instead.
...
const STYLE_ENUM_VALUES = {
  ...
  page_system: new Set(['clinical-rail', 'board-ledger', 'editorial-field', 'command-canvas', 'lab-plate', 'investor-thesis', 'pulse-plate', 'none']),
  ...
  header_mode: new Set(['bar', 'stack', 'eyebrow', 'lab-clean', 'lab-card', 'pulse-plate']),
  ...
  cards_mode: new Set(['feature-left', 'staggered-row', 'aligned-row']),
  ...
  footer_mode: new Set(['standard', 'source-line', 'pulse-plate', 'none']),
  summary_callout_mode: new Set(['default', 'lab-box', 'pulse-plate']),
...
function applyDeckStyle(basePreset, data, presetName) {
  ...
  // forcePresetName pins the house style. Resolving the treatment from the
  // forced name too keeps tokens and treatment from ever disagreeing -- a
  // corporate palette wearing another preset's page system is worse than
  // either preset on its own.
  const forced = forcePresetName(presetName);
  preset.style_preset = forced;
  const treatment = PRESET_TREATMENTS[forced] || {};
  ...
  // deck_style can otherwise re-theme a forced preset from inside the outline.
  // style_preset is already ignored above; palette_key and font_pair would
  // still repaint it, so drop them while a preset is forced.
  if (FORCED_PRESET_NAME) {
    for (const key of ['style_preset', 'palette_key', 'font_pair']) {
      if (deckStyle[key] !== undefined) {
        console.warn(
          `[pptxgenjs] deck_style.${key}='${deckStyle[key]}' в outline игнорируется; ` +
          `этот форк собирает всё в '${FORCED_PRESET_NAME}'.`,
        );
        delete deckStyle[key];
      }
    }
  }
...
function normalizeSlide(spec, outlineDir) {
  ...
  // Map common outline aliases onto the keys this renderer actually reads.
  // Without it an outline that says "content"/"points"/"items"/"stats"
  // renders as a header over blank space, with a zero exit code and no
  // warning -- the failure mode that looks like a broken theme.
  const aliasRepairs = applySlotAliases(out);
  if (aliasRepairs.length) {
    console.warn(
      `[pptxgenjs] слайд '${String(out.title || out.type || 'без имени').slice(0, 48)}': ` +
      `починены ключи outline -> ${aliasRepairs.join(', ')}`,
    );
  }
...
function renderSlide(pptx, pSlide, slide, preset) {
  ...
  slides.ensurePageChrome(pSlide, preset);
...
async function main() {
  ...
  // A preset may retune the whole type scale (see setTypeRamp in slides.js).
  // Set once per deck; null restores the house scale for every other preset.
  slides.setTypeRamp(preset.type_ramp || null);

  const pptx = new PptxGenJS();
  pptx.defineLayout({ name: 'PPTX_SKILL_16x9', width: slides.SLIDE_W, height: slides.SLIDE_H });
  pptx.layout = 'PPTX_SKILL_16x9';
  // Document properties are visible in PowerPoint's File > Info panel, so the
  // placeholder wording follows the preset's labels like everything else.
  const metaFallback = {
    title: slides.label(preset, 'untitledDeck'),
    author: slides.label(preset, 'deckAuthor'),
  };
  pptx.title = String(data.title || metaFallback.title);
  ...
  const neutralMetadata = {
    author: String(documentMetadata.author || metaFallback.author).trim() || metaFallback.author,
  ...
  // A slide that still has no payload after alias repair renders as a header
  // over blank space. That used to pass silently with exit code 0; name it.
  const emptySlides = normalized
    .map((s, i) => (isPayloadEmpty(s) ? i + 1 : 0))
    .filter(Boolean);
  if (emptySlides.length) {
    console.warn(
      `[pptxgenjs] слайдов с заголовком, но без содержимого: ${emptySlides.length} ` +
      `(${emptySlides.join(', ')}). Проверьте ключи outline по ` +
      'references/outline_schema.md и pulse/DESIGN_RULES.md.',
    );
  }
  ...
  console.log(`Wrote ${produced} (${normalized.length} slides, preset=${preset.style_preset})`);
}
